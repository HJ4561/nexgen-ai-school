// src/modules/admin/pages/Canteen/index.jsx
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/50 rounded-b-2xl">
          <button onClick={onClose} disabled={loading} className="px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all disabled:opacity-50">Cancel</button>
          <button onClick={onSave} disabled={loading || !formData.name || !formData.category || !formData.price} className="px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl hover:from-blue-700 hover:to-blue-800 shadow-md shadow-blue-600/25 transition-all disabled:opacity-50 flex items-center gap-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/50 rounded-b-2xl">
          <button onClick={onClose} disabled={loading} className="px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all disabled:opacity-50">Cancel</button>
          <button onClick={onSave} disabled={loading || !formData.menu_item || !formData.quantity} className="px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl hover:from-blue-700 hover:to-blue-800 shadow-md shadow-blue-600/25 transition-all disabled:opacity-50 flex items-center gap-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          <h3 className="font-semibold text-gray-800 text-lg flex items-center gap-2">
            {type === "menu" ? <Utensils className="w-5 h-5 text-orange-600" /> : type === "order" ? <ShoppingCart className="w-5 h-5 text-blue-600" /> : <Tag className="w-5 h-5 text-purple-600" />}
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            {type === "menu" && (
              <>
                <div><p className="text-xs text-gray-500">Name</p><p className="font-medium text-gray-800">{item.name || "—"}</p></div>
                <div><p className="text-xs text-gray-500">Category</p><p className="font-medium text-gray-800">{item.category?.name || item.category || "—"}</p></div>
                ...
                <div className="col-span-2"><p className="text-xs text-gray-500">Status</p><Badge className={getStatusColor(item.is_available !== false, item.stock || 0)}>{getStatusLabel(item.is_available !== false, item.stock || 0)}</Badge></div>
                <div className="col-span-2"><p className="text-xs text-gray-500">Description</p><p className="text-gray-600">{item.description || "—"}</p></div>
              </>
            )}
            {type === "order" && (
              <>
                <div><p className="text-xs text-gray-500">Item</p><p className="font-medium text-gray-800">{item.menu_item?.name || "—"}</p></div>
                <div><p className="text-xs text-gray-500">Student</p><p className="font-medium text-gray-800">{item.student?.name || item.student || "—"}</p></div>
                ...
                <div><p className="text-xs text-gray-500">Status</p><Badge className={ORDER_STATUS[item.status]?.color || "bg-gray-50 text-gray-700 border-gray-200"}>{item.status ? ORDER_STATUS[item.status]?.label || item.status : "—"}</Badge></div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 text-lg">Update Order Status</h3>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1">Status</label>
            <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
              <option value="placed">Placed</option>
              <option value="preparing">Preparing</option>
              <option value="ready">Ready</option>
              <option value="served">Served</option>
        ...
        <div className="flex gap-2 mt-6 pt-4 border-t border-gray-100">
          <button onClick={onClose} disabled={loading} className="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all disabled:opacity-50">Cancel</button>
          <button onClick={() => onUpdate(order.id, status)} disabled={loading || status === order.status} className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl hover:from-blue-700 hover:to-blue-800 shadow-md shadow-blue-600/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
...
const Canteen = () => {
  ...
  const getOrderStatus = (status) => ORDER_STATUS[status] || { label: status || "Unknown", color: "bg-gray-50 text-gray-700 border-gray-200" };
  ...
  const currentItems = activeTab === "menu" ? filteredMenuItems : activeTab === "orders" ? filteredOrders : filteredCategories;
  ...
  const stats = useMemo(() => {
    ...
    const lowStockItems = menuItems.filter(i => i.is_available === true && i.stock && i.stock <= 5).length;
  ...
  const handleSaveCategory = async () => {
    if (!categoryFormData.name) { showToast("Please enter a category name", "error"); return; }
    setSaving(true);
    try {
      const payload = { name: categoryFormData.name, description: categoryFormData.description || "", color: categoryFormData.color || "default" };
      if (modalMode === "edit" && selectedCategory) {
        const response = await api.patch(`${CATEGORIES_API}${selectedCategory.id}/`, payload);
        setCategories(categories.map(c => c.id === selectedCategory.id ? { ...c, ...response.data } : c));
  ...
  const handleSaveMenuItem = async () => {
    ...
    try {
      ...
      if (modalMode === "edit" && selectedItem) {
        ...
        setMenuItems(menuItems.map(i => i.id === selectedItem.id ? { ...i, ...response.data } : i));
  ...
  const handleAddOrder = async () => {
    ...
    } catch (error) { showToast(error.response?.data?.detail || "Failed to create order", "error"); }
    finally { setSaving(false); }
  };
  ...
  const handleUpdateOrderStatus = async (orderId, status) => {
    ...
    try {
      ...
      setOrders(orders.map(o => o.id === orderId ? { ...o, ...response.data } : o));
      showToast(`Order status updated to ${ORDER_STATUS[status]?.label || status}`, "success");
      setStatusModalOpen(false);
      setSelectedOrder(null);
    } catch (error) { showToast(error.response?.data?.detail || "Failed to update status", "error"); }
    finally { setSaving(false); }
  };
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const apiUrl = activeTab === "menu" ? MENU_ITEMS_API : activeTab === "orders" ? ORDER_ITEMS_API : CATEGORIES_API;
      await api.delete(`${apiUrl}${deletingItem.id}/`);
      if (activeTab === "menu") setMenuItems(prev => prev.filter(item => item.id !== deletingItem.id));
      else if (activeTab === "orders") setOrders(prev => prev.filter(order => order.id !== deletingItem.id));
      else setCategories(prev => prev.filter(cat => cat.id !== deletingItem.id));
  ...
  // ─── Tabs ─────────────────────────────────────────────────────────────
  const tabs = [
    { id: "menu", label: "Menu Items", icon: <Utensils className="w-4 h-4" />, count: menuItems.length },
    { id: "orders", label: "Orders", icon: <ShoppingCart className="w-4 h-4" />, count: orders.length },
    { id: "categories", label: "Categories", icon: <Tag className="w-4 h-4" />, count: categories.length },
  ];
    ...
    return (
      <FadeIn>
        <div className="space-y-8">
          <PageHeader title="Canteen Management" subtitle="Manage canteen menu, orders, and categories" breadcrumbs={["Admin", "Canteen"]} />
          <div className="flex flex-col items-center justify-center h-96">
            <div className="animate-spin rounded-full h-16 w-16 border-4 border-blue-100 border-t-blue-600"></div>
            <p className="mt-6 text-sm text-gray-500 font-medium">Loading canteen data...</p>
          </div>
        </div>
      </FadeIn>
    );
    ...
    <FadeIn>
      <div className="space-y-8">
        <PageHeader 
          ...
          subtitle={`Manage canteen menu, orders, and categories${menuItems.length > 0 ? ` — ${menuItems.length} items, ${orders.length} orders` : ""}`}
          ...
          action={
            <div className="flex items-center gap-2.5">
              ...
              <button
                onClick={() => {
                  if (activeTab === "menu") { setModalMode("add"); setMenuFormData({ name: "", category: "", price: "", stock: "", is_available: true, description: "" }); setMenuModalOpen(true); }
                  else if (activeTab === "orders") { setOrderFormData({ menu_item: "", quantity: 1, student: "", status: "placed" }); setOrderModalOpen(true); }
                  else { setModalMode("add"); setCategoryFormData({ name: "", description: "", color: "default" }); setCategoryModalOpen(true); }
                }}
        ...
        {error && (
          <div className="text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 flex items-start gap-2">
            ...
            <div><p className="font-medium">Error loading canteen</p><p className="text-amber-600">{error}</p></div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="p-4 border-l-4 border-l-blue-500">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Menu Items</p>
            <p className="text-2xl font-bold text-gray-800">{stats.totalItems}</p>
            <p className="text-xs text-gray-400 mt-1">{stats.availableItems} available</p>
          </Card>
          <Card className="p-4 border-l-4 border-l-emerald-500">
            ...
            <p className="text-xs text-gray-400 mt-1">{stats.pendingOrders} pending</p>
          </Card>
              ...
              <div className="text-center py-16">
                <div className="flex flex-col items-center gap-4">
                  <div className="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center">
                    {activeTab === "menu" ? <Utensils className="w-10 h-10 text-gray-400" /> :
                     activeTab === "orders" ? <ShoppingCart className="w-10 h-10 text-gray-400" /> :
                    ...
                    if (activeTab === "menu") {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-blue-50/30 transition-colors duration-150 group">
                          <td className="px-4 py-3.5"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center"><Utensils className="w-4 h-4 text-orange-600" /></div><span className="font-medium text-gray-900">{item.name || "—"}</span></div></td>
                          ...
                          <td className="px-4 py-3.5"><Badge className={stock === 0 ? "bg-red-50 text-red-700 border-red-200" : stock <= 5 ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}>{stock}</Badge></td>
                          <td className="px-4 py-3.5"><Badge className={getStatusColor(status, stock)}>{getStatusLabel(status, stock)}</Badge></td>
                          <td className="px-4 py-3.5 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => { setSelectedItem(item); setDetailsModalOpen(true); }} className="p-2 rounded-lg hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-all" title="View"><Eye className="w-4 h-4" /></button>
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-blue-50/30 transition-colors duration-150 group">
                          <td className="px-4 py-3.5"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center"><ShoppingCart className="w-4 h-4 text-orange-600" /></div><span className="font-medium text-gray-900">{getMenuItemName(item.menu_item)}</span></div></td>
                          <td className="px-4 py-3.5 text-gray-700">{item.student?.name || "—"}</td>
                          ...
                          <td className="px-4 py-3.5"><Badge className={`${status.color} flex items-center gap-1`}>{ORDER_STATUS_ICONS[item.status]}{status.label}</Badge></td>
                          <td className="px-4 py-3.5 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => { setSelectedOrder(item); setStatusModalOpen(true); }} className="p-2 rounded-lg hover:bg-emerald-50 text-emerald-600 transition-all" title="Update status"><CheckCircle className="w-4 h-4" /></button>
                              <button onClick={() => { setSelectedOrder(item); setDetailsModalOpen(true); }} className="p-2 rounded-lg hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-all" title="View"><Eye className="w-4 h-4" /></button>
                              <button onClick={() => setDeletingItem(item)} className="p-2 rounded-lg hover:bg-rose-50 text-gray-500 hover:text-rose-600 transition-all" title="Delete"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ...
                      const itemCount = menuItems.filter(i => i.category === item.id).length;
                      return (
                        <tr key={item.id} className="hover:bg-blue-50/30 transition-colors duration-150 group">
                          <td className="px-4 py-3.5"><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-lg ${colorClass} flex items-center justify-center`}><Tag className="w-4 h-4" /></div><span className="font-medium text-gray-900">{item.name || "—"}</span></div></td>
                          <td className="px-4 py-3.5 text-sm text-gray-600 truncate max-w-xs">{item.description || "—"}</td>
                          <td className="px-4 py-3.5"><Badge className="bg-blue-50 text-blue-700 border-blue-200 text-xs">{itemCount}</Badge></td>
                          <td className="px-4 py-3.5 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => { setSelectedCategory(item); setDetailsModalOpen(true); }} className="p-2 rounded-lg hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-all" title="View"><Eye className="w-4 h-4" /></button>
                              <button onClick={() => { setModalMode("edit"); setSelectedCategory(item); setCategoryFormData({ name: item.name || "", description: item.description || "", color: item.color || "default" }); setCategoryModalOpen(true); }} className="p-2 rounded-lg hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-all" title="Edit"><Edit className="w-4 h-4" /></button>
                              <button onClick={() => setDeletingItem(item)} className="p-2 rounded-lg hover:bg-rose-50 text-gray-500 hover:text-rose-600 transition-all" title="Delete"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
      ...
      {/* Modals */}
      <CategoryModal isOpen={categoryModalOpen} onClose={() => { setCategoryModalOpen(false); setSelectedCategory(null); }} mode={modalMode} formData={categoryFormData} setFormData={setCategoryFormData} onSave={handleSaveCategory} loading={saving} />
      <MenuItemModal isOpen={menuModalOpen} onClose={() => { setMenuModalOpen(false); setSelectedItem(null); }} mode={modalMode} formData={menuFormData} setFormData={setMenuFormData} onSave={handleSaveMenuItem} loading={saving} categories={categories} />
      <OrderModal isOpen={orderModalOpen} onClose={() => { setOrderModalOpen(false); setOrderFormData({ menu_item: "", quantity: 1, student: "", status: "placed" }); }} formData={orderFormData} setFormData={setOrderFormData} onSave={handleAddOrder} loading={saving} menuItems={menuItems} />
      <ViewDetailsModal isOpen={detailsModalOpen} onClose={() => { setDetailsModalOpen(false); setSelectedItem(null); setSelectedOrder(null); setSelectedCategory(null); }} item={selectedItem || selectedOrder || selectedCategory} type={activeTab} />
      <UpdateOrderStatusModal isOpen={statusModalOpen} onClose={() => { setStatusModalOpen(false); setSelectedOrder(null); }} order={selectedOrder} onUpdate={handleUpdateOrderStatus} loading={saving} />

      {/* Delete Confirmation */}
      {deletingItem && (
        <ConfirmDialog open={true} title={`Delete ${activeTab === "menu" ? "Menu Item" : activeTab === "orders" ? "Order" : "Category"}`} message={activeTab === "menu" ? `Are you sure you want to delete "${deletingItem.name}"?` : activeTab === "orders" ? `Are you sure you want to delete this order for "${getMenuItemName(deletingItem.menu_item)}"?` : `Are you sure you want to delete "${deletingItem.name}"?`} confirmLabel="Delete" onConfirm={handleDelete} onCancel={() => setDeletingItem(null)} loading={saving} />
      )}
