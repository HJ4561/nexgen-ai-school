// src/modules/admin/pages/Rooms.jsx
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        ...
        <p className="mt-4 text-sm sm:text-base text-gray-500 font-medium">Loading rooms...</p>
      </div>
    );
  }
  ...
  return (
    <FadeIn>
      <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 lg:px-6">
        ...
        <PageHeader 
          title="Rooms"
          subtitle="Manage all rooms and facilities"
          actions={
        ...
        {errored && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 sm:p-4">
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-700">Error loading rooms</p>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
          <Card className="p-3 sm:p-4 md:p-5 border-l-4 border-l-blue-500">
          ...
          <Card className="p-3 sm:p-4 md:p-5 border-l-4 border-l-green-500">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Total Capacity</p>
            <p className="text-xl sm:text-2xl md:text-3xl font-bold text-green-600">{stats.totalCapacity}</p>
              ...
              <div className="text-center py-8 sm:py-12 px-4">
                <div className="flex flex-col items-center gap-2 sm:gap-3">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gray-100 flex items-center justify-center">
                    <DoorOpen className="w-6 h-6 sm:w-8 sm:h-8 text-gray-400" />
                  </div>
                  <p className="text-sm sm:text-base font-medium text-gray-500">No rooms found</p>
                  <p className="text-xs sm:text-sm text-gray-400">Add a room to get started</p>
                  <Button variant="primary" size="sm" onClick={openAdd} className="mt-2">
                    <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" /> Add Room
                  </Button>
                </div>
              </div>
            ) : (
              <table className="w-full min-w-[500px] sm:min-w-[600px]">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-3 sm:px-4 py-2.5 sm:py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Room</th>
                    <th className="text-left px-3 sm:px-4 py-2.5 sm:py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider hidden sm:table-cell">Location</th>
                    <th className="text-left px-3 sm:px-4 py-2.5 sm:py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Capacity</th>
                    ...
                    <tr key={room.id} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <span className="text-sm sm:text-base font-medium text-gray-800 truncate max-w-[100px] sm:max-w-[150px]">
                            {room.name || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}
                          </span>
                        </div>
                        {/* Location shown below on small screens */}
                        <div className="sm:hidden mt-1 flex items-center gap-1 text-xs text-gray-500">
                          <MapPin className="w-3 h-3" />
                          <span className="truncate">{room.location || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</span>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 hidden sm:table-cell">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm text-gray-600">{room.location || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</span>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                        <span className="font-semibold text-sm sm:text-base text-gray-800">{room.capacity || 0}</span>
                      </td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
          ...
          <div className="relative w-full max-w-[95%] sm:max-w-md bg-white rounded-xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-xl sm:rounded-t-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3 sm:space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-200 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm sm:text-base"
              ...
              <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-2 sm:gap-3 pt-2">
                <Button 
                  ...
                  onClick={closeModal} 
                  disabled={saving}
                  className="w-full sm:w-auto min-h-[36px] sm:min-h-[40px]"
                >
                  Cancel
                </Button>
                <Button 
      ...
      {detailModalOpen && selectedRoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
          ...
          <div className="relative w-full max-w-[95%] sm:max-w-md bg-white rounded-xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="relative px-4 sm:px-6 py-4 sm:py-6 bg-gradient-to-r from-blue-600 to-purple-600 rounded-t-xl sm:rounded-t-2xl">
              <button 
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white text-xl sm:text-2xl lg:text-3xl font-bold shadow-lg">
                  {selectedRoom.name?.charAt(0).toUpperCase() || "R"}
                </div>
                <div>
                  <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-white">
                    {selectedRoom.name || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}
                  </h3>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="bg-gray-50 rounded-lg sm:rounded-xl p-3 sm:p-4">
                  ...
                  <p className="text-sm sm:text-base font-semibold text-gray-800">
                    {selectedRoom.location || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg sm:rounded-xl p-3 sm:p-4">
                  <div className="flex items-center gap-2 text-gray-500 mb-1">
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-end gap-2 sm:gap-3 bg-gray-50/50 rounded-b-xl sm:rounded-b-2xl">
              <Button 
                variant="outline" 
                size="sm"
                onClick={() => { closeDetailModal(); openEdit(selectedRoom); }}
                className="w-full sm:w-auto min-h-[36px] sm:min-h-[40px]"
              >
                <Edit className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" /> Edit
              </Button>
              <Button 
                variant="primary" 
                size="sm"
