...
import { 
  CheckCircle, AlertCircle, X, Send, RefreshCw, Bell, 
  Mail, Users, Clock, Search, Trash2
} from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <Card className="p-4 border-l-4 border-l-emerald-500">
        ...
        <p className="text-xs text-gray-400 mt-1">Read notifications</p>
      </Card>
      ...
      <Card className="p-4 border-l-4 border-l-purple-500">
        <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Unread Count</p>
        <p className="text-2xl font-bold text-purple-600">{stats.unreadCount || 0}</p>
        <p className="text-xs text-gray-400 mt-1">Total unread</p>
      </Card>
    </div>
  );
};
...
const NotificationFilters = ({ 
  ...
  return (
    <Card className="p-4 sm:p-6 border-gray-100">
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-sm"
          />
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={onRefresh}
            disabled={refreshing}
            className="px-4 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all flex items-center gap-2 disabled:opacity-50"
          >
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="px-4 py-2.5 text-sm font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-xl hover:bg-blue-100 transition-all flex items-center gap-2"
            >
      ...
      <div className="text-center py-16">
        <div className="flex flex-col items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Notifications will appear here when sent.</p>
          </div>
        </div>
      </div>
    );
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Title / Message</th>
            ...
            {pageItems.map((notification) => (
              <tr key={notification.id} className={`hover:bg-blue-50/30 transition-colors group ${!notification.is_read ? "bg-blue-50/10" : ""}`}>
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center">
                      <Bell className={`w-4 h-4 ${!notification.is_read ? "text-amber-600" : "text-gray-400"}`} />
                    </div>
                    <div>
                ...
                <td className="px-4 py-3.5 text-sm text-gray-600">
                  {notification.user || notification.user_name || "—"}
                </td>
                <td className="px-4 py-3.5">
                  <Badge className={`${getTypeBadge(notification.type)} text-xs px-2.5 py-1`}>
...
const SendNotificationDrawer = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-right-4 duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          <h3 className="font-semibold text-gray-800 text-lg">Send Notification</h3>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1">Recipient <span className="text-red-500">*</span></label>
            <select
              value={form.user || ""}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/50 rounded-b-2xl">
          ...
          >
            {loading ? <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent" /> : <Send className="w-4 h-4" />}
            Send Notification
          </button>
        </div>
      </div>
...
export default function NotificationManagement() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setError("Notifications endpoint not found. Please check the API configuration.");
  ...
  const handleSendNotification = async () => {
    if (!sendForm.user || !sendForm.title || !sendForm.message) {
    ...
    try {
      ...
      const response = await api.post(NOTIFICATIONS_API, payload);
      setNotifications([response.data, ...notifications]);
      showToast("Notification sent successfully", "success");
      setIsDrawerOpen(false);
      setSendForm({ user: "", title: "", message: "", type: "sms" });
    } catch (error) {
      console.error("Failed to send notification:", error);
      showToast(error.response?.data?.detail || "Failed to send notification", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.delete(`${NOTIFICATIONS_API}${deletingItem.id}/`);
      setNotifications(notifications.filter(n => n.id !== deletingItem.id));
  ...
  if (loading && notifications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-96">
        ...
        <p className="mt-6 text-sm text-gray-500 font-medium">Loading notifications...</p>
      </div>
    );
  }
  ...
  if (error && notifications.length === 0) {
    return (
      <div className="p-10 text-center">
        <div className="text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 flex items-start gap-2 max-w-md mx-auto">
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-gray-50 min-h-screen">
      {/* Toast */}
      {toast.visible && (
        <div className="fixed top-4 right-4 z-50 max-w-sm w-full bg-white rounded-lg shadow-lg border border-gray-200 p-4 flex items-start gap-3 animate-in slide-in-from-top-4 duration-300">
          {toast.type === 'success' ? (
      ...
      {deletingItem && (
        <ConfirmDialog
          ...
          message={`Are you sure you want to delete "${deletingItem.title || 'this notification'}"? This action cannot be undone.`}
          confirmLabel="Delete"
