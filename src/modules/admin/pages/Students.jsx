// src/modules/admin/pages/Students.jsx
import React, { useState, useCallback } from "react";
import { 
  Search, Plus, Edit, Trash2, MapPin, Phone, Mail, 
  Calendar, Users, User, X, RefreshCw, AlertCircle, 
...
const Students = () => {
  ...
  }, [editingStudent, update, create, showToast]);
  ...
  }, [deletingStudent, remove, showToast]);
      ...
      <FadeIn>
        <div className="space-y-6 md:space-y-8 px-4 sm:px-6 md:px-8">
          ...
          <div className="flex flex-col items-center justify-center h-64 md:h-96">
            ...
            <p className="mt-4 md:mt-6 text-sm text-gray-500 font-medium">Loading students...</p>
          </div>
        </div>
      </FadeIn>
    );
  ...
  return (
    <FadeIn>
      <div className="space-y-6 md:space-y-8 px-4 sm:px-6 md:px-8">
        <PageHeader 
          title="Students" 
          subtitle={`Manage all students${records.length ? ` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${records.length} total` : ""}`}
          breadcrumbs={["Admin", "Students"]}
          icon={GraduationCap}
          action={
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <Card className="p-3 md:p-4 border-l-4 border-l-blue-500">
            ...
            <p className="text-[10px] md:text-xs text-gray-400 mt-0.5 md:mt-1">All students</p>
          </Card>
          ...
          <Card className="p-3 md:p-4 border-l-4 border-l-purple-500">
            <p className="text-[10px] md:text-xs font-medium text-gray-500 uppercase tracking-wider">Classes</p>
            <p className="text-xl md:text-2xl font-bold text-purple-600">
              {new Set(records.map(s => s.class_obj?.name || s.class_obj)).size}
            </p>
            <p className="text-[10px] md:text-xs text-gray-400 mt-0.5 md:mt-1">Unique classes</p>
          </Card>
        </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="font-semibold text-gray-900 text-sm truncate">{student.name || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</p>
                                <p className="text-xs text-gray-500 truncate">{student.email || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</p>
                              </div>
                            </div>
                            <div className="flex flex-wrap items-center gap-1.5 mt-2">
                              ...
                              <span className="text-xs text-gray-600">{student.class_obj?.name || student.class_obj || "Not assigned"}</span>
                              <Badge className={`${STATUS_BADGE[student.status] || "bg-gray-50 text-gray-600 border-gray-200"} text-[10px] flex items-center gap-1`}>
                                <span>{STATUS_ICON[student.status] || "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢"}</span>
                          ...
                          <td className="px-4 py-3.5">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="font-semibold text-gray-900 text-sm truncate max-w-[150px]">{student.name || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</p>
                                <p className="text-xs text-gray-500 truncate max-w-[150px]">{student.email || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3.5">
                            <Badge className="bg-indigo-50 text-indigo-700 border-indigo-200 text-xs whitespace-nowrap">
                              #{student.admission_no || "N/A"}
                            </Badge>
                          </td>
                          <td className="px-4 py-3.5">
                            <span className="text-sm text-gray-700 truncate max-w-[120px] block">
                              {student.class_obj?.name || student.class_obj || "Not assigned"}
                            </span>
                          </td>
                          <td className="px-4 py-3.5">
                            <Badge className={`${STATUS_BADGE[student.status] || "bg-gray-50 text-gray-600 border-gray-200"} text-xs flex items-center gap-1.5 px-2.5 py-1 whitespace-nowrap`}>
                              <span>{STATUS_ICON[student.status] || "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢"}</span>
                              {student.status ? student.status.charAt(0).toUpperCase() + student.status.slice(1) : "Unknown"}
                            </Badge>
                          </td>
