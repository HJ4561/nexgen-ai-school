// src/modules/admin/pages/EventManagement.jsx
...
const EventFilters = ({ search, setSearch, filterStatus, setFilterStatus }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100 p-4 flex flex-col sm:flex-row items-center gap-3">
      ...
      <select
        ...
        className="px-4 py-2 bg-gray-50 border-none rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none min-w-[140px] w-full sm:w-auto"
      >
...
const EventTable = ({ 
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100 p-6 text-center">
                ...
                <tr key={event.id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{event.name || event.event_name || "Ã¢â‚¬â€"}</p>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm text-gray-600 truncate max-w-[120px]">{event.location || event.venue || "Ã¢â‚¬â€"}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-500">
                        {event.event_date ? new Date(event.event_date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : "Ã¢â‚¬â€"}
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5 text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => onViewParticipants(event)} className="p-2 rounded-lg hover:bg-blue-50 text-blue-600 transition-colors" title="View Participants">
                        <Users className="w-4 h-4" />
                      </button>
                      <button onClick={() => onEdit(event)} className="p-2 rounded-lg hover:bg-blue-50 text-blue-600 transition-colors" title="Edit Event">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => onDelete(event)} className="p-2 rounded-lg hover:bg-red-50 text-red-600 transition-colors" title="Delete Event">
...
const EventStats = ({ stats }) => {
  ...
  return (
    <StaggerGroup className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
    ...
    <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100 p-5">
      ...
      <div className="mt-4 flex flex-col sm:flex-row items-center gap-2">
        ...
        <select
          ...
          className="flex-1 w-full px-3.5 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
        >
      ...
      <div className="flex gap-2 mt-3">
        <Button variant="outline" fullWidth onClick={onViewParticipants} disabled={!selectedEvent}>
          <Users size={14} className="mr-2" />
          View
        </Button>
        <Button variant="primary" fullWidth onClick={onAddParticipant} disabled={!selectedEvent}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-2 bg-gray-50/50 rounded-b-2xl">
          ...
          <Button variant="primary" onClick={onSave} disabled={loading || !formData.student_id} className="w-full sm:w-auto flex items-center gap-2">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {participants.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  ...
                  <button onClick={() => onRemove(p.id)} className="p-2 rounded-lg hover:bg-red-50 text-red-600 transition-colors" title="Remove Participant">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 transition-colors" type="button">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-2 bg-gray-50/50 rounded-b-2xl">
          <Button variant="outline" onClick={onClose} disabled={loading} className="w-full sm:w-auto" type="button">
            Cancel
          </Button>
          <Button variant="primary" onClick={onSave} disabled={loading || !isFormValid} className="w-full sm:w-auto flex items-center gap-2" type="button">
...
const EventManagement = () => {
  ...
  // --- Render ----------------------------------------------------------
  return (
    <div className="p-4 flex flex-col gap-5 min-h-screen bg-gray-50/50">
      {/* Toast */}
      {toast && (
      ...
      <FadeIn y={10} duration={0.5}>
        <PageHeader
          title="Event Management"
          subtitle={`Manage school events and participant registrations${events.length > 0 ? ` Ã¢â‚¬â€ ${events.length} events` : ""}`}
          breadcrumbs={["Admin", "Events"]}
          action={
