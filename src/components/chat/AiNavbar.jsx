...
export default function AiNavbar({
  ...
  return (
    <header
      ref={barRef}
      className="relative flex flex-wrap items-center justify-between px-3 sm:px-4 md:px-6 py-2 sm:py-2.5 md:py-3 border-b shrink-0 gap-2"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 md:gap-3 min-w-0 flex-1">
        ...
        <div className="leading-tight min-w-0">
          <p className="text-xs sm:text-sm md:text-base font-semibold text-white tracking-tight truncate">
            AI Workspace
          </p>
          <p className="text-[10px] sm:text-[11px] text-white/40 hidden sm:block">
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 shrink-0">
        ...
        <div className="hidden sm:block w-px h-6 mx-0.5 sm:mx-1" style={{ background: 'rgba(255,255,255,0.1)' }} />

        {/* User Info */}
        <div className="flex items-center gap-1.5 sm:gap-2 pl-0 sm:pl-1">
          <div
            className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-semibold text-white shrink-0"
            style={{
              background: INDIGO,
          ...
          <div className="leading-tight hidden sm:block">
            <p className="text-xs sm:text-sm md:text-base font-medium text-white truncate max-w-[80px] sm:max-w-[100px] md:max-w-[120px]">
              {userName || 'User'}
            </p>
            <p className="text-[10px] sm:text-[11px] text-white/40 capitalize">{userRole}</p>
          </div>
        </div>
        ...
        <button
          onClick={onLogout}
          className="p-1.5 sm:p-2 rounded-lg text-white/40 hover:text-red-400 transition-colors"
          onMouseEnter={(e) =>
          ...
          aria-label="Log out"
        >
          <LogOut size={17} className="sm:w-[18px] sm:h-[18px]" />
        </button>
      </div>
    </header>
  );
